...
apt update
apt upgrade -y
apt install -y curl pip git python3-venv pkg-config libicu-dev sudo podman zip jq wget gh devscripts
...
python3 -m venv ./
source ./bin/activate
pip install -U platformio adafruit-nrfutil grpcio-tools
